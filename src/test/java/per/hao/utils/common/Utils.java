...
//     * @param encoding 编码
     ...
     * @return 返回去掉参数的url
     */
    public static String checkHasParas(String url, List<NameValuePair> nvps, HttpConfig config) throws UnsupportedEncodingException {
        // 检测url中是否存在参数
        if (url.contains("?") && url.indexOf("?") < url.indexOf("=")) {
            Map<String, Object> map = buildParas(url.substring(url.indexOf("?") + 1));
            config.getParams(map);
            map2HttpEntity(nvps, config);
...
//    public static HttpEntity map2HttpEntity(List<NameValuePair> nvps, Map<String, Object> map, String encoding) throws UnsupportedEncodingException {
    public static HttpEntity map2HttpEntity(List<NameValuePair> nvps, HttpConfig config) throws UnsupportedEncodingException {

        Map<String, Object> map = config.getParams();
        String encoding = config.inenc();

        String contentType = "application/getRequestBody";
            ...
            for (Entry<String, Object> entry : map.entrySet()) {
                if (SPECIAL_ENTITIY.contains(entry.getKey())) {//判断是否在之中
                    ...
                    if (ENTITY_STRING.equals(entry.getKey())) {//string
                        ...
                        break;
                    } else if (ENTITY_JSON.equals(entry.getKey())) {//getRequestBody
                        entity = new StringEntity(String.valueOf(entry.getValue()), encoding);
//                        String contentType = "application/getRequestBody";
...
//							builder.addPart(String.valueOf(getParams.get(ENTITY_MULTIPART+".name")) + count++, fileBody);
