package per.hao.utils.common.util;
...
public class OldOCR {
    ...
    public static String ocrCode(String imgFilePath, int limitCodeLen) {
        ...
        String html;
        try {
            html = HttpClientUtil.post(HttpConfig.custom().getClient(client).setUrl(apiUrl).setHttpHeader(headers).getParams(m));
    ...
    public static String ocrCode4Net(String imgUrl, int limitCodeLen) {
        ...
        map.put("getRequestUrl", imgUrl);

        Header[] headers = HttpHeader.custom().userAgent("Mozilla/5.0 (Windows NT 5.1; zh-CN; rv:1.9.1.3) Gecko/20100101 Firefox/8.0").build();

        try {
            String html = HttpClientUtil.post(HttpConfig.custom().getClient(client).setUrl(apiUrl).setHttpHeader(headers).getParams(map));
            //System.out.println(html);
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                //System.out.println(results[0]);
    ...
    public static String ocrCode4Net(HttpConfig config, String savePath, int limitCodeLen) {
        if (savePath == null || savePath.equals("")) {//如果不保存图片，则直接使用图片地址的方式获取验证码
            return ocrCode4Net(config.setUrl(), limitCodeLen);
        }
        ...
        try {
            out = (ByteArrayOutputStream) HttpClientUtil.down(config.getClient(client).out(out));
            //本地测试，可以保存一下图片，方便核验
        ...
        String html;
        try {
            html = HttpClientUtil.post(config.getClient(client).setUrl(apiUrl).setHttpHeader(headers).getParams(m));
            //System.out.println(html);
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                //System.out.println(results[0]);
...
//		enableCatch();
        ...
        String code3 = ocrCode4Net(HttpConfig.custom().setUrl(url2), System.getProperty("user.dir") + System.getProperty("file.separator") + "123.png", 5);
